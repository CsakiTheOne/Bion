...
<body>
    ...
    <div class="container w-50 mx-auto my-5 bg-light">
        <div class="row">
            <div class="col-3 mt-4 text-center">
                <img src="../img/profile.png" style="width: 100px; height: 100px;">
            </div>
            <div class="col-9 my-3 text-dark text-left">
                <h2><i class="fas fa-signature mr-3"></i> <?php echo $_SESSION['username']; ?></h2>
                <h2><i class="fas fa-envelope mr-3"></i> <?php echo $_SESSION['email']; ?></h2>
                <?php
                $data = callProc("CommentGetByUsername", "'{$_SESSION['username']}'");
                $comments = count($data);
                ?>
                <h2><i class="far fa-comments mr-3"></i> <?php echo $comments ?></h2>
            </div>
        </div>
        <hr style="border: 1px solid black;">
        <div class="row">
            <div class="col-3 mb-2">
                <?php $data = callProc("ThemeGetByUserUniqIdWithCategory", "'{$_SESSION['username']}'");
                $countOfData = count($data);
                ?>
                <h3 class="text-dark">Posztjaim (<?php echo $countOfData ?>)</h3>
            </div>
        </div>
        <?php
        $data = callProc("ThemeGetByUserUniqIdWithCategory", "'{$_SESSION['username']}'");
        foreach ($data as $value) {
            include "../php/topic/topicComponentProfile.php";
        }
        unset($value);
        ...
        if (isset($_POST['show'])) {
            ...
            echo "<script>
                window.location.assign('showPost.php');
            </script>";
        }
        ?>
    </div>
